...
export function MixRecipeSheetForm() {
   ...
   async function onSubmit(values: z.infer<typeof formSchema>) {
      const response = await addMixRecipe({...values, recipeId});
   ...
   return (
      <Sheet>
         ...
         <div className='grid grid-cols-1 md:grid-cols-2 px-4 gap-2 justify-between'>
            {!isLoading && recipes.map((r: any,i: number) => (
               <Button 
                  key={i} 
                  variant={'outline'} 
                  onClick={() => handleClick(r.id)}
                  className={cn('bg-transparent border-2 ease-in-out duration-100', recipeId == r.id && 'bg-blue-500 text-white dark:bg-blue-500'
               )}>
                  {r.name}
               </Button>
            ))}
